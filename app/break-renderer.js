...
window.onload = async (event) => {
  ...
  const frictionEnabled = strictMode && await window.settings.get('breakSkipFrictionEnabled')
  ...
  document.querySelector('#friction-skip').onclick = async () => {
    const currentTotalCount = await window.settings.get('breakSkipFrictionTotalCount')
    const currentSequentialCount = await window.settings.get('breakSkipFrictionSequentialCount')

    if (currentTotalCount < Number.MAX_SAFE_INTEGER) {
      await window.settings.saveSettings('breakSkipFrictionTotalCount', currentTotalCount + 1)
    }
    if (currentSequentialCount < Number.MAX_SAFE_INTEGER) {
      await window.settings.saveSettings('breakSkipFrictionSequentialCount', currentSequentialCount + 1)
    }
