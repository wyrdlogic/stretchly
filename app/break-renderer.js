...
window.onload = async (event) => {
  ...
  document.querySelector('#friction-skip').onclick = async () => {
    await window.settings.saveSettings('breakSkipFrictionTotalCount',
      (await window.settings.get('breakSkipFrictionTotalCount')) + 1)
    await window.settings.saveSettings('breakSkipFrictionSequentialCount',
      (await window.settings.get('breakSkipFrictionSequentialCount')) + 1)
    await window.breaks.finishBreak()
  }
